...
class LeadController extends Controller
{
    ...
    {   
        ...
        $companies = Company::get();
         $assign_user=User::where('company_id',Auth::user()->company_id)->get();
      
        return view("admin.lead.index",compact("leadTypeArr","currency","type","leadTypeName","companies","countries","assign_user"));
    }
    ...
    public function getSchedule($id){
         // -------added by Manali Joshi at 05/12/23----------
           $activityList = Activity::with("lead")->orderBy("id","desc")->where('lead_id', $id)->get();
       
        $clientinfo= Lead::where('iid',$id)->first();
        //---
    return view("admin.lead.schedule",compact('clientinfo','activityList'));
    }
    ...
    public function edit(Lead $lead)
    {
        ...
        $editForm = view("admin.lead._form",compact("companies","lead","leadTypeArr","customeArr","currency","leadItem","countries","assign_user"))->render();
        return response()->json([
            "data" => $editForm
        ]);
        ...
        if (count(array_diff($requiredColumns, $fileColumns)) > 0) {
            ...
            foreach ($data as $key => $row) {
                ...
                //     $vRemark = $row[12];
                    ...
                    $bulklead->company_name =$vCompany;
                    $bulklead->vWhatsapp = $vWhatsapp;
                    $bulklead->vEmail = $vEmail;
                    $bulklead->vTitle = $vTitle;
                    $bulklead->vPlateform = $vPlateform;
                    
                    $bulklead->vCompany = auth()->user()->company_id;
                    $bulklead->vleadType = "general";
                    $bulklead->vPipeline = 6;
